...
export const BottomNav = () => {
  ...
  return (
    <nav
      ...
      className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-[#0D1220]/95 backdrop-blur-md border-t border-slate-200/80 dark:border-[rgba(148,163,184,0.12)] shadow-xs pb-[env(safe-area-inset-bottom,0px)]"
    >
            ...
            >
              <div
                className={`relative flex items-center justify-center h-8 w-11 rounded-xl transition-colors ${
                  active
                    ? 'bg-[rgba(124,108,242,0.12)] dark:bg-[rgba(124,108,242,0.18)]'
                    : 'bg-transparent'
                }`}
              >
                <Icon size={19} strokeWidth={active ? 2.2 : 1.75} className={active ? 'text-[#7C6CF2] dark:text-[#8B7CF6]' : ''} />
              </div>
              <span
